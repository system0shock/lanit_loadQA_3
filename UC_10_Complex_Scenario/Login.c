Login()
{
	...
	lr_start_transaction("UC10_open_login_form");
	...
	lr_end_transaction("UC10_open_login_form",LR_AUTO);

	lr_start_transaction("UC10_login");
	...
	if (HTTP_rc < 400){
		...
		lr_end_transaction("UC10_login",LR_PASS);
	} else {
		lr_log_message("auth fail");
		lr_end_transaction("UC10_login",LR_FAIL);
	}
